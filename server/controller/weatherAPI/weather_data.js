...
module.exports = async (req, res) => {
  try {
    ...
    await weather_data.destroy({ where: { city: id } });
    ...
    function weatherApi(xy) {
      ...
      xy.map((el) => {
        ...
        const base_time = String(Number(moment().format("HH"))) - 1 + "00";
        const nx = el[0];
        const ny = el[1];
        const nz = el[2];
        const dataType = "XML";
        ...
        // url
        const all_url = url1 + "?serviceKey=" + key + "&pageNo=1&numOfRows=10000" + "&dataType=" + dataType + "&base_date=" + day + "&base_time=" + base_time + "&nx=" + nx + "&ny=" + ny;
        console.log(all_url);
    ...
    if (result.length !== 0) weatherApi(result);
    if (result.length !== 0) return res.status(201).json({ data: result, message: "받지못한 날씨 좌표가 있습니다." });

    return res.status(201).json({ message: "해당 city의 모든 날씨를 받아왔습니다" });
  } catch (err) {
    console.log(err);
  }
};
