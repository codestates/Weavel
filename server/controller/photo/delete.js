...
module.exports = async (req, res) => {
  try {
    ...
    if (!id) {
      return res
        .status(409)
        .json({ message: "지우려는 사진 id 정보가 없습니다." });
    } else if (!weather) {
      return res
        .status(409)
        .json({ message: "지우려는 날씨 정보가 없습니다." });
    } else if (!date) {
      return res
        .status(409)
        .json({ message: "지우려는 날짜 정보가 없습니다." });
    } else if (!area) {
      return res
        .status(409)
        .json({ message: "지우려는 지역 정보가 없습니다." });
    } else if (!filename) {
      return res
        .status(409)
        .json({ message: "지우려는 사진파일 이름 정보가 없습니다." });
    }

    // DB 데이터 검색
    const path = await photo.findOne({
      where: {
        id: id,
        userId: userId,
        weather: weather,
    ...
    // 파일 정보 삭제
    const info = await photo.destroy({
      where: {
        id: id,
        userId: userId,
        weather: weather,
