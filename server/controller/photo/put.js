...
module.exports = async (req, res) => {
  try {
    ...
    // 사진 id 데이터가 없을때 결과값
    if (!id) {
      res.status(409).json({ massage: "지우려는 사진 id를 입력하세요" });
    }

    // 업데이트하려는 정보 칼럼을 조회하지 못하였을 때
    const path = await photo.findOne({ where: { id: id, userId: userId } });
    if (!path) {
      res.status(409).json({ massage: "이미 삭제 되었거나, 존재하지않는 정보입니다." });
    }
    ...
    if (fs.existsSync(path.image)) {
      ...
      res.status(409).json({ massage: "이미 삭제 되었거나, 존재하지 않는 파일입니다." });
    }

    const update = await photo.update({ image: newpath, filename: newfilename }, { where: { id: id, userId: userId } });
    if (!update) {
      res.status(409).json({ massage: "사진의 데이터가 저장되지 않았습니다." });
    }

    res.status(201).json({ massage: "사진이 수정되었습니다." });
  } catch (err) {
    console.log("err");
  }
};
